...
impl MarkManager {
    ...
    pub fn set_global_mark(&mut self, mark_char: char, line: usize, column: usize, filename: Option<PathBuf>) -> Result<(), String> {
        match mark_char {
            'A'..='Z' => {
    ...
    pub fn cleanup_for_closed_buffer(&mut self, closed_filename: Option<&std::path::PathBuf>) {        
        ...
        if let Some(filename) = closed_filename {
            self.global_marks.retain(|_mark, mark_data| {
                mark_data.filename.as_ref() != Some(filename)
            });
            
            // Remove jump list entries for the closed file
            self.jump_list.retain(|entry| {
                entry.filename.as_ref() != Some(filename)
            });
    ...
    pub fn list_marks(&self, local_marks: &std::collections::HashMap<char, (usize, usize)>) -> Vec<(char, usize, usize, Option<&std::path::PathBuf>)> {
...
mod tests {
    ...
    fn test_global_marks() {
        ...
        // Set a global mark
        assert!(manager.set_global_mark('A', 20, 10, filename.clone()).is_ok());
    ...
    fn test_clear_marks() {
        ...
        manager.set_global_mark('A', 20, 10, Some(std::path::PathBuf::from("test.txt"))).unwrap();
    ...
    fn test_get_jump_list() {
        ...
        assert_eq!(jump_list[1].filename, Some(std::path::PathBuf::from("test.txt")));
    }
    ...
    fn test_cleanup_for_closed_buffer() {
        ...
        assert!(manager.jump_list.iter().all(|entry| entry.filename != file1));
    }
}