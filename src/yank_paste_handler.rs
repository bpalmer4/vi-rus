...
impl YankPasteHandler {
    pub fn execute_yank(
        ...
        let (text, register_type) = Self::get_yank_content(&yank_type, controller.current_document());
        
        // Store in register
        controller.register_manager.store_in_register(register, text.clone(), register_type);
    ...
    pub fn execute_visual_yank(controller: &mut Controller, register: Option<char>) {
        if let Some(selection) = controller.visual_selection.as_ref() {
            let text = VisualModeHandler::get_selected_text(selection, controller.current_document());
    ...
    fn get_yank_content(yank_type: &YankType, document: &Document) -> (String, RegisterType) {
        match yank_type {
            ...
            YankType::BigWordBackward => (document.yank_big_word_backward(), RegisterType::Character),
            YankType::ToEndOfWord => (document.yank_to_end_of_word(), RegisterType::Character),
            YankType::ToEndOfBigWord => (document.yank_to_end_of_big_word(), RegisterType::Character),
            ...
            YankType::ToFirstNonWhitespace => (document.yank_to_first_non_whitespace(), RegisterType::Character),
            ...
            YankType::UntilChar(target) => (document.yank_until_char(*target), RegisterType::Character),
            YankType::UntilCharBackward(target) => (document.yank_until_char_backward(*target), RegisterType::Character),
            YankType::FindChar(target) => (document.yank_find_char(*target), RegisterType::Character),
            YankType::FindCharBackward(target) => (document.yank_find_char_backward(*target), RegisterType::Character),
        }
    }
